...
END`;
const DESTINATION_INTERNAL_SQL = `EXISTS (
  SELECT 1 FROM agents direction_dst
  WHERE direction_dst.extension IN (cdr.dst, cdr.dstchannel_ext)
)`;
const SOURCE_INTERNAL_SQL = `EXISTS (
  SELECT 1 FROM agents direction_src
  WHERE direction_src.extension IN (cdr.src, cdr.channel_ext)
)`;
const DIRECTION_CASE_SQL = `CASE
  WHEN ${DESTINATION_INTERNAL_SQL} THEN 'incoming'
  WHEN ${SOURCE_INTERNAL_SQL} THEN 'outgoing'
  ELSE 'unknown'
END`;
...
const buildWhere = (filters = {}) => {
  ...
  if (filters.direction === 'incoming') {
    clauses.push(DESTINATION_INTERNAL_SQL);
  }
  if (filters.direction === 'outgoing') {
    clauses.push(SOURCE_INTERNAL_SQL);
    clauses.push(`NOT ${DESTINATION_INTERNAL_SQL}`);
  }
  if (filters.q) {
...
const getCdr = async (filters) => {
  ...
  const rowsResult = await pool.query(
    `SELECT cdr.id,
            ...
            ${DIRECTION_CASE_SQL} AS direction,
...
const getDashboardStats = async (filters) => {
  ...
  const [totals, perAgent, perDay, perStatus, perHour] = await Promise.all([
    pool.query(`SELECT COUNT(*)::int AS total_calls,
                       ...
                       COALESCE(SUM(CASE WHEN ${DIRECTION_CASE_SQL} = 'incoming' THEN 1 ELSE 0 END),0)::int AS incoming_calls,
                       COALESCE(SUM(CASE WHEN ${DIRECTION_CASE_SQL} = 'outgoing' THEN 1 ELSE 0 END),0)::int AS outgoing_calls
                ${CDR_JOIN_SQL}
                ${where}`, values),
  ...
  return {
    ...
    incomingCalls: totals.rows[0].incoming_calls,
    outgoingCalls: totals.rows[0].outgoing_calls,
